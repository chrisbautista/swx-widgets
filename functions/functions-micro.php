 ...
 **/
 $href_permalink = the_permalink();
function neat_trim($str, $n, $delim='...') {
   $len = strlen($str);
   if ($len > $n) {
       preg_match('/(.{' . $n . '}.*?)\b/', $str, $matches);
       return rtrim($matches[1]) . $delim;  //.'<p><a href="' . the_permalink(); . '">Continue Reading</a></p>' .;
   }
   else {
       return $str;
   }
...
function neat_trim_code($str, $n, $delim='...') {
   ...
   if ($len > $n) {
	   ...
	   //preg_match('/(.{' . $n . '}.*?)\b/', $str, $matches);
       return substr($str, 0, $offset) . $delim; //. '<p><a href="'. the_permalink(); .'">Continue Reading</a></p>' .;
   }
   else {
       return $str;
   }
}
// chop the amount of lenght down 
...
function is_child($pageID) {
	global $post;
	if( is_page($pageID) || ($post->post_parent==$pageID) ) {
               return true;
	} else {
               return false;
	}
...
//Display Related Posts by Same Author   http://www.wpbeginner.com/wp-tutorials/how-to-display-related-posts-by-same-author-in-wordpress/
function get_related_author_posts() {
    global $authordata, $post;

    $authors_posts = get_posts( array( 'author' => $authordata->ID, 'post__not_in' => array( $post->ID ), 'posts_per_page' => 5 ) );
	if(!empty($authors_posts)){
    $output = '<div class="insights-front"><ul>';
    foreach ( $authors_posts as $authors_post ) {
        $output .= '<li class="no-bullet"><a href="' . get_permalink( $authors_post->ID ) . '" class="link-black"><h4>' . apply_filters( 'the_title', $authors_post->post_title, $authors_post->ID ) . '&nbsp;<span class="news-glyphicon-play"></span></h4></a></li>';
		}
		$output .= '</ul></div>';
	}
    return $output;
...
/*-----------------------------------------------------------------------------------*/
/* grey fluid background
/*-----------------------------------------------------------------------------------*/

function open_grey_fluid_background($atts, $content=null) {
...
add_shortcode( 'open_grey_fluid_background', 'open_grey_fluid_background' );

function end_grey_fluid_background($atts, $content=null) {
...
add_shortcode( 'end_grey_fluid_background', 'end_grey_fluid_background' );


/*-----------------------------------------------------------------------------------*/
/* ACF helper - speed up checking for a field value on a page, pulling options if it wasn't found 
/*-----------------------------------------------------------------------------------*/

function check_acf_field($field, $options_field = null) {


	// if options field not set, use field name
	if(null === $options_field) {
		$options_field = $field;
	}
	...
	// check in options
	elseif(get_field($options_field,'option')) {
		$return = get_field($options_field,'option');
		}
	else {
		$return = '<span class="bg-danger">Value for '.$options_field.' not found. Please configure via options panel.</span>';
		}
	return $return;
}

/*-----------------------------------------------------------------------------------*/
/* ACF helper - count rows because count() doesn't seem to work
/*-----------------------------------------------------------------------------------*/

function acf_count($field) {
	$i = 0;
	if( have_rows($field) ):
